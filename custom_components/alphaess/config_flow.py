...
async def validate_input(hass: HomeAssistant, data: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        # authenticate() itself calls getESSList(); calling both back-to-back
        # can trigger 6053. With raise_on_error, one direct read distinguishes
        # rejected credentials from a valid account with no bound systems.
...
class AlphaESSInverterSubentryFlowHandler(ConfigSubentryFlow):
    ...
    def _get_coordinator(self):
        """Get the loaded coordinator so subentry calls share its API gate."""
    ...
    async def async_step_user(
        ...
        if user_input is not None:
            ...
            try:
                ...
                result = await coordinator.async_request_verification_code(
                    sys_sn, check_code
                )
                _LOGGER.info(
                ...
                # Successful mutation endpoints return data:null, which the
                # library exposes as None. With raise_on_error enabled, success
                # is therefore "no exception", not a non-None payload.
    ...
    async def async_step_verify(
        ...
        if user_input is not None:
            ...
            try:
                coordinator = self._get_coordinator()
                result = await coordinator.async_bind_system(self._sysSn, code)
                _LOGGER.info(
                ...
                if e.code == 6003:
                    # The requested end state already exists. Treat this as an
                    # idempotent success and create the HA subentry.
                    _LOGGER.info("System %s was already bound", self._sysSn)
                    accepted = True
                else:
            ...
            if accepted:
                ...
                return self.async_create_entry(
                    ...
                    unique_id=f"{SUBENTRY_TYPE_INVERTER}_{self._sysSn}",
                    data={
    ...
    async def async_step_reconfigure(
        ...
        if user_input is not None:
            ...
            if user_input.get("confirm_unbind"):
                ...
                try:
                    coordinator = self._get_coordinator()
                    result = await coordinator.async_unbind_system(serial)
                    _LOGGER.info(
                    ...
                    if e.code == 6005:
                        # Already unbound is the requested end state; remove the
                        # stale HA subentry instead of trapping the user in it.
                        _LOGGER.info("System %s was already unbound", serial)
                        accepted = True
                    else:
